package com.netvine.weeklyreport.interceptor;


import com.netvine.weeklyreport.common.Common;
import com.netvine.weeklyreport.utils.JSONResult;
import com.netvine.weeklyreport.utils.JsonUtils;
import com.netvine.weeklyreport.utils.RedisOperator;
...
public class UserTokenInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (StringUtils.isBlank(userId) || StringUtils.isBlank(userToken)) {
            returnErrorDdata(response, JsonUtils.objectToJson(JSONResult.errorMsg("请先登录!")));
        }else {
            //读取redis用户信息
            ...
            if(StringUtils.isBlank(userUUID)){
                ...
                if(!userUUID.equals(userToken)){
                    returnErrorDdata(response, JsonUtils.objectToJson(JSONResult.errorMsg("已经异地登录，请重新登录!")));
                }
            }
        }
